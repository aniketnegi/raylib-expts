...
int main(void) {
  ...
  while (!WindowShouldClose()) {
    ...
    targetScale1 = 1.0f + (fabs(targetPosition1.x - position1.x) / 50.0f +
                           fabs(targetPosition1.y - position1.y) / 50.0f);
    targetScale2 = 1.0f + (fabs(targetPosition2.x - position2.x) / 50.0f +
                           fabs(targetPosition2.y - position2.y) / 50.0f);
    ...
    DrawCircleV(position2, 10 * scale2, GREEN);
